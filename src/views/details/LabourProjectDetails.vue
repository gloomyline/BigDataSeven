  ...
  <div class="dispatch">
    ...
    <div class="table boxall">
      <p class="thead">
        ...
        <span class="tilte">项目劳动力配置率</span>
      </p>
      <div class="chartBox">
...
export default {
  ...
  methods: {
    ...
    ratioFormat(row, column) {
	    return row.ratio.toString().trim().concat("%")
	  },
    tableRowClassName({row, rowIndex}) {
	    //console.log("rowrowindex",row,rowIndex)
	    if(row.ratio>=90){
	      return 'textcolorGreen'
	    }else if(row.ratio<90 && row.prate>=80){
    ...
    async getTableData(){
      ...
      var res = await LabourServicesNewApi.fetchfindLabour(this.$route.params.ny,this.$route.params.id)
      console.log("劳动配置表格",res)
      if(!res){
        return
      }
      this.tableData1 = res
    ...
    echarts(finish, remained) {
      ...
      let str
      if(finish){
        str = Number(finish) /(Number(finish)+(Number(remained))*100) === 'NaN' ? `0` : (Number(finish) / (Number(finish)+(Number(remained)))*100)
      }else {
        str = '0'
      }
