  ...
  <div class="production">
    ...
    <div class="chartContainerFather">
      <dv-border-box-10 class="chartContainer">
        ...
        <div class="chartContentSon">
          <div class="chartTit hoverhand" @click="goto()">项目大型机械设备(单位:台)</div>
...
export default {
  ...
  methods: {
    async _requestData() {
      ...
      //大施组比对
      const res   = await equipmentApi.fetchfindBigConstructionData(this.date); 
      console.log("大施组比对",res)
      this.bigCompare = res
    },
    goto(){
      this.$router.push({ name: "BigMainEquipmentDetails" })
    },
    _drawEcharts() {
      const self = this;
      // 项目大型机械设备
