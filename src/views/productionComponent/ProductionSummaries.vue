          ...
          <div class="chartContentSon" style="width: 50%;">
            <dv-border-box-10 class="chartContChild">
                <p>
                  ...
                  >为<span class="sred">红色警示</span>项目
                </p>
              </dv-border-box-10>
...
export default {
  ...
  methods: {
    handleScroll() {
      //$.this.scrollTop
      console.log(
        "this.$refs.scroll.$el handldScroll-----",
        this.$refs.scroll.$el.children[1].scrollTop
      );
    ...
    getDeptNumName() {
      ...
      productionNewApi.fetchGetDeptNumNameData().then(res => {
        ...
        console.log("this.DeptArr111111111", this.DeptArr);
      });
    },
    //数组比较
    compare(key) {
    ...
    async selfSupport() {
      ...
      this.seriesData.forEach(item => {
        item.data = [];
        let data = [
          parseInt(Number(res.data.proinfo.monthly.plan)),
          parseInt(Number(res.data.proinfo.yearly.plan)),
          parseInt(Number(res.data.proinfo.sofar.remained))
        ];
        let downData = [
          parseInt(Number(res.data.proinfo.monthly.finished)),
          parseInt(Number(res.data.proinfo.yearly.finished)),
          parseInt(Number(res.data.proinfo.sofar.finished))
        ];
        if (item.name === "计划") {
    ...
    async getLagList() {
      ...
      const newZhb = zhb.map(item => {
        ...
        return [
          `<span style="color:${color};">${item[0]}</span>`,
    ...
    async initData() {
      ...
      if (
        ...
        this.allCompany.data.productionValue
      ) {
        Object.keys(this.allCompany.data.productionValue).forEach(item => {
          this.DeptArr.forEach(i => {
            if (i.value === item) {
              ...
              if (
                this.total === 0 ||
                Number(this.allCompany.data.productionValue[item] === 0)
              ) {
                str = 0;
              } else {
                str = parseInt(
                  Number(
                    this.allCompany.data.productionValue[item] / this.total
                  ) * 100
                );
              ...
              var obj = {
                ...
                name: `${i.name}:${
                  this.allCompany.data.productionValue[item]
                }(${str}%)`
              };
              console.log("obj", obj);
    ...
    onTableClick(row) {
      ...
      this.lagList &&
        this.lagList.length > 0 &&
        this.lagList.forEach((item, index) => {
