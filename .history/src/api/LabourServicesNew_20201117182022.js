...
/*  async fetchLabelteamworkrateData(date) {
  ...
  //描述
  async fetchconStatistics(date) {
    const response = await axios.get('/labour/getContractsStatistics', { params: { month: date}});
    return response.data;
  },
  //指定等级的数据详情
  async fetchfindAllSubconsByLevel(page,limit,level) {
    const response = await axios.get('/labour/findAllSubconsByLevel', { params: {page:page,limit:limit, level: level}});
