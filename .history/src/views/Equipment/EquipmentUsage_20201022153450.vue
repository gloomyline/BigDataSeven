...
export default {
  ...
  methods:{
    async initData(){
      ...
      this.usingRate(res["设备利用率"])
      this.completeRate(res["设备完好率"])
      this.expendRate(res["燃油消耗率"])
    },
    price(data){
      var deptId = []
      var deptName = []
    ...
    completeRate(data){
      ...
      this.echarts("completeRate", xData, seriesData, '单位(L/H)')
    },
    expendRate(data){
      var deptId = []
      var deptName = []
      ...
      this.echarts("expendRate", xData, seriesData, '单位(台)')
    },
    echarts(id, xData, seriesData, yTitle, projectIdArr) {
      var myChart = echarts.init(document.getElementById(id));
      var _that = this;
      var option = {
        color:["#56CA95","#FFBA55","#DE1A37","#FFBA55"],
        legend:{
          show:true,
          textStyle:{
            color:"#fff"
