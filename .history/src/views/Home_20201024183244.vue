  ...
  <div class="mainbox">
    <ul class="clearfix">
      ...
      <li>
        ...
        <dv-border-box-13 class="bar">
          <div id="comconent">拼命加载中...</div>
        </dv-border-box-13>
        <div class="map" >
      ...
      <li>
        <dv-border-box-1 class="section-box">
          <div class="alltitle" @click="goRouter('Finance')">公司预算管理</div>
...
export default {
  ...
  methods: {
    ...
    async echarts(pointArr, provinceProJectsArr) {
      ...
      // console.log("_this.homeMapCompanyData",_this.homeMapCompanyData)
      var MapCompanyData = _this.homeMapCompanyData
      var comName = []
      var provinces=[]
      var comContent = []
      ...
      let provincesDataIndex = provinces.map((item)=>{
        var itemlist =item.map((provinces)=>{
            ...
            return provincesIndex[provinces]
        })
        return itemlist
    ...
    _this.timerCarousel=setInterval(Carousel(),8000)
    console.log("_this  timerCarousel",_this.timerCarousel)
    //timerCarousel()
    ...
    echarts_5() {
      ...
      myChart.on('click', function (param){
        ...
        self.$router.push({ name: "BigMainEquipmentDetails" });
        }        	
      });
      window.addEventListener("resize", function() {
        myChart.resize();
    ...
    echarts_51() {
      ...
      const self = this;
      var netWorth = [self.equipment.netValue,self.equipment.unusedNetValue]
      const option = {
        title: [
          {
            text: "自有主要机械设备使用情况",
        ...
        series: [
          {
            ...
            label: {
              ...
              formatter:function(params){
                ...
                return `${params.name}:${params.value}<br>净值${netWorth[params.dataIndex]}`
              },
              padding: 7,
              height:15,
              align: 'left',
      ...
      myChart.on('click', function (param){
        ...
        if(name=="空闲"){
        //   self.$router.push({ name: 'bigMainEquipment'}).catch(data => {});
        self.$router.push({ name: "TZEquipmentDetails" });
        }        	
      });
      window.addEventListener("resize", function() {
        myChart.resize();
    ...
    echarts_finance1(){
      ...
      for (var item in planMapData ){
        ...
        outnum.push(planMapData[item].outnum)
        undonerate.push(planMapData[item].undonerate)
        outnumrate.push(planMapData[item].outnumrate)
        compalist.push(planMapData[item].completed+planMapData[item].undone+planMapData[item].outnum)
      ...
      // [
        ...
        //        //  console.log("我的参数",params)
			  //  console.log("total[params.params]",params)
        //        return `${total[params.dataIndex]}亿`
        //       }
        //      },
        //      barWidth : 30
        ...
        //       show:true,
        //       formatter: function(params) {
        //       return `${undonerate[params.dataIndex]}%`
        //     }
        //     },
        //     barWidth : 30
        //   },
        ...
        //       show:true,
        //       formatter: function(params) {
        //       return `${outnumrate[params.dataIndex]}%`
        //     }
        //     },
        //     barWidth : 30
		    //   }
      ...
      var option = {
        ...
        color: ["rgb(2,2,2,0)", "#27d08a", "#e62d2d", "#FFD700", "#0fb4d6"],
        grid: {
          top:'28%',
          left: '5%',
          right: '5%',
        ...
        tooltip: {
            ...
            formatter:function(params){
              ...
              if(params[0].name == "两金压降"){
                namestr=`压降目标:${total[params[0].dataIndex]}<br>`
              }else {
                namestr=`目标:${total[params[0].dataIndex]}<br>`
              }
              
              console.log("---------------completed",completed[params[0].dataIndex])

              if(completed[params[0].dataIndex] !=0){
                completedstr =`完成: ${completed[params[0].dataIndex]}<br>`
              }else {
                completedstr=""
              }

              console.log("---------------undone",undone[params[0].dataIndex])

              if(undone[params[0].dataIndex] !=0){
                undonestr =`未完: ${undone[params[0].dataIndex]}<br>`
              }else {
                undonestr=""
              }
              
              console.log("---------------outnum",outnum[params[0].dataIndex])
              if(outnum[params[0].dataIndex] !=0){
                outnumstr =`超额: ${outnum[params[0].dataIndex]}<br>`
              }else {
                outnumstr=""
              }
              
              
              // if(completed[params.dataIndex]<0){

              // }


              return `${namestr}${completedstr}${undonestr}${outnumstr}`
            }
        },
        xAxis: [
          {
    ...
    echarts_finance2(){
      ...
      for (var item in profitMapData ){
        ...
        undone.push(profitMapData[item].undone)
        undonerate.push(profitMapData[item].undonerate)
        outnum.push(profitMapData[item].outnum)
        outnumrate.push(profitMapData[item].outnumrate)
        compalist.push(profitMapData[item].completed+profitMapData[item].undone+profitMapData[item].outnum)

       }

      let completedABS = completed.map((item)=>{
        return Math.abs(item)
      })
       console.log("xAxisarr",xAxisarr)
       console.log("completedABS",completedABS)
      var option = {
        legend: {
          data: ['已完', '未完','超额'],
          textStyle: {
    ...
    echarts_economy(){
      ...
      for(var i in economyData){
        ...
        completedUnpriced.push(economyData[i].completedUnpriced)
      }
      ...
      var option = {
        ...
        // color: ["rgb(2,2,2)", "#27d08a", "#e62d2d", "#FFD700", "#0fb4d6"],
        color: [ "#0fa0d6","#27d08a", "#FFD700", "#e62d2d" ],
        legend: {
            show:true,
            textStyle: {
              color: "rgba(255,255,255,.6)",
      ...
      var option = {
        ...
        legend: {
          ...
          formatter: function(params) {
            ...
            return params === "完成" ? `月计划:${parseInt(planed)}` : "";
          }
        },
        series: [
          {
    ...
    echarts_32(monthly) {
      ...
      var option = {
        ...
        legend: {
          ...
          selectedMode: false,
          formatter: function(params) {
            return params === "完成" ? `年计划:${parseInt(planed)}` : "";
          }
        },
        series: [
          {
    ...
    echarts_33(monthly) {
      ...
      var option = {
        ...
        legend: {
          ...
          selectedMode: false,
          formatter: function(params) {
            return params === "完成" ? `总产值:${parseInt(planed)}` : "";
          }
        },
        series: [
          {
  ...
  destroyed() {
    ...
    console.log(this.timerCarousel)
