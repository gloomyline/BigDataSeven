<template>
  <div class="dispatch">
    ...
    <div class="table boxall">
      <p class="thead">
        <img class="img" src="@/assets/images/u563.png" />
        <span class="tilte">节点滞后</span>
      </p>
      <div class="chartBox">
        <!-- 节点滞后 -->
        <!-- 里程碑线路 -->
        <div class="title">里程碑线路</div>
        <el-table
        :data="milerNodeList"
        style="width: 100%">
        <el-table-column
      ...
      <div class="title">主要线路</div>
        ...
        <el-table-column
         ...
         <!-- <el-table :data="milerNodeList" style="width: 100%">
          <el-table-column
            type="index"
            label="序号" >
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getNodeListData() {
      const res1 = await DispatchNewApi.fetchedNodeList(`${this.$route.params.ny}`,`${this.$route.params.id}`,1);
      // console.log("nodeList",res)
      this.milerNodeList = res1.data
      const res2 = await DispatchNewApi.fetchedNodeList(`${this.$route.params.ny}`,`${this.$route.params.id}`,2);
      this.keyLineNodeList = res2.data
      const res3 = await DispatchNewApi.fetchedNodeList(`${this.$route.params.ny}`,`${this.$route.params.id}`,3);
