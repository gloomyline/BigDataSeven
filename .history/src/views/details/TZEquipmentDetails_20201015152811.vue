...
export default {
  ...
  methods: {
    ...
    filterUsingState(value, row){
      console.log("row------1",row)
      // return row.children[0].usingState ===value
      return row.children.forEach((item,index)=>{
        console.log("item.usingState",typeof item.usingState)
        console.log("value",typeof value)
        console.log("item.usingState===value",Boolean(item.usingState===value))
        return item.usingState===value
      })
