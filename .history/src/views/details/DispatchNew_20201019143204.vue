...
export default {
  ...
  methods: {
    ...
    async getNodeListData() {
      ...
      this.milerNodeList = res.data

      const res2 = await DispatchNewApi.fetchedNodeList(`${this.$route.params.ny}`,`${this.$route.params.id}`,1);
      const res3 = await DispatchNewApi.fetchedNodeList(`${this.$route.params.ny}`,`${this.$route.params.id}`,1);
      this.keyLineNodeList
      this.primaryLineNodeList
