...
export default {
  ...
  methods: {
    ...
    async finishlmonth() {
      ...
      console.log("departProduction"+this.$route.params)
      if(res && res.data) {
        this.echarts_31(res.data.monthly.finished, res.data.monthly.remained,res.data.monthly.planed)
        this.echarts_32(res.data.yearly.finished, res.data.yearly.remained,res.data.yearly.planed)
        this.echarts_33(res.data.sofar.finished, res.data.sofar.remained,res.data.sofar.planed)
      } else {
        this.echarts_31(0, 0,0)
        this.echarts_32(0, 0,0)
        this.echarts_33(0, 0,0)
      }
    },
    goBack(res) {
      if (res) {
    ...
    echarts_31(finish, remained,planed) {
    ...
    echarts_32(finish, remained,planed) {
      let str = (Number(finish) / (Number(finish)+(Number(remained)))*100).toFixed(2) === 'NaN' ? `0` : (Number(finish) / (Number(finish)+(Number(remained)))*100).toFixed(2)
      // 基于准备好的dom，初始化echarts实例
      let rendercolor="red"
      if(str>=90){
    ...
    echarts_33(finish, remained,planed) {
