...
export default {
  ...
  methods: {
    async initData() {
      ...
      this.allEquipment = response.concat()
      this.tableData2 = response.concat()
      this.usingEquipment = response.map((item)=>{
      ...
      console.log("using后  respon",this.allEquipment)
      this.freeEquipment= response.map((item2)=>{
        // console.log("filter item",item)
