...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      const res = await DispatchNewApi.fetchPlanfinishlMonthData1(`${this.$route.params.ny}`,`${this.$route.params.id}`,1);
      //console.log("getTableData",res)
      // this.tableData = res.data
    },
    // 节点滞后模块   { ny:ny,projectid: pid, type:type}
    async getNodeListData() {
      const res = await DispatchNewApi.fetchedNodeList(`${this.$route.params.id}`);
      //console.log("getTableData",res)
      // this.tableData = res.data
    },
    async projectInfo() {
