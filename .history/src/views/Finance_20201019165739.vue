...
import fundCollection from "@/views/finance/fundCollection";
import districtBudget from "@/views/finance/districtBudget";
import shuangQingManagement from "@/views/finance/shuangQingManagement";
export default {
  ...
  data() {
    return {
      ...
      current: 0,
      tabs: ['资金回款', '片区预算', '双清管理' ],  
      currentComponent: 'fundCollection',
      componentList: ['fundCollection', 'districtBudget', 'shuangQingManagement'],
    };
  },
  components:{fundCollection,districtBudget,shuangQingManagement},
  watch:{
  ...
  methods: {
     initData(){
      // 获取项目计价回款数据
      financeApi.fetchShuangQing(this.ny).then((data)=>{
        this.financeValuationData = data;   
        // this.financeValuation()
      })
